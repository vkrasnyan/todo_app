...
class TestTaskCollaboratorModel:
    async def test_fields(self, async_session: AsyncSession) -> None:
        current_fields_name = [i.name for i in TaskCollaborator.__table__.columns]
        related_fields = [
            i._dependency_processor.key for i in TaskCollaborator.__mapper__.relationships
        ]
        all_model_fields = current_fields_name + related_fields
        schema_fields_name = {"task_id", "user_id", "can_read", "can_update", "task", "user"}
